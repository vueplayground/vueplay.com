				...
				>
					...
					>
						<Text
							...
							class="hash"
							style="width:100%"
						>Checkout - {{product?.name || application?.title || product?.slug}}</Text>
					</Box>
					<Box
						borderBottomColor="{`default`:{`xs`:{`light`:`color-mix(in oklch, var(--color-neutral) 90%, white)`,`dark`:`color-mix(in oklch, var(--color-neutral) 90%, black)`}}}"
						paddingTop="{`default`:{`xs`:{`light`:`0px`}}}"
					...
					>
						<Text
							...
							class="hash"
							style="width:100%"
						>Price: ${{application?.price / 100}} inc.</Text>
					</Box>
					<Box
						v-if="customer"
...
<script>
	...
	export default {
		...
		computed: {
			icon() {
				return this.application?.icon?.replace('width', 'data-width')
		...
		methods: {
			async getApplication() {
				...
				this.application = apps?.data?.[0];
			},
			async getProduct() {
				const res = await fetch(`https://market.vueplay.io/products/${this.application?.product_id}`);
			...
			async checkout() {
				try {
					this.order = await this.market.service('orders')
						.checkout({
							origin: location.origin + `/market/${this.category || 'all'}/${this.slug}/${this.version ? this.version + '/' : ''}checkout`,
