				...
				>
					...
					>
						<Text
							...
							class="hash"
							style="width:100%"
						>Success - {{product?.name || application?.title || product?.slug}}</Text>
					</Box>
					<Box
						padding="{`default`:{`xs`:{`light`:`0px`}}}"
						marginTop="{`default`:{`xs`:{`light`:`50px`}}}"
					>Thank you for purchasing {{product?.name || application?.title || product?.slug}}.<br /><br />You just got full access to your purchase!</Box>
					<Link
						title="Open"
						...
						justifyContent="{`default`:{`xs`:{`light`:`center`}}}"
						:route="`/market/${category || 'all'}/${slug}${version ? '/' + version : ''}`"
					>
					</Link>
				</Box>
			</Box>
